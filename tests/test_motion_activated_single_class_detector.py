...
def test_no_motion_no_detection(detectors, frame):
    ...
    motion_activated_detector = MotionActivatedSingleClassDetector(detector, motion_detector)
...
def test_slack_time_no_motion(detectors, frame):
    ...
    motion_activated_detector = MotionActivatedSingleClassDetector(detector, motion_detector, slack=10)
    motion_activated_detector.detect(frame)
    detector.detect.assert_called_once_with(frame)
    detector.reset_mock()
