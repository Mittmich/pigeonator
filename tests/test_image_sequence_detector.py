...
def test_single_class_sequence_has_reached_consensus(long_sequence_detector, mock_event_manager, empty_array):
...
def test_single_class_sequence_detector_resets_after_detection(small_sequence_detector, mock_event_manager, empty_array):
    seq = SingleClassSequenceDetector(minimum_number_detections=3, detector=small_sequence_detector)
    seq.add_event_manager(mock_event_manager)
